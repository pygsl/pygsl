...
PyGSL_copy_array_to_array(PyArrayObject *dst, PyArrayObject *src,  enum pygsl_transform_mode mode)
{
	...
	if(
	    (flag = PyGSL_STRIDE_RECALC(PyArray_STRIDE(src, 0), size, &stride1) != GSL_SUCCESS) ||
	...
	if(iscomplex){
		...
		/* Todo: add test for this bug, was z1, z2 before */
		...
		return gsl_blas_ccopy(&c2.vector, &c1.vector);
	     }
	    break;
	    }
	}else{
...
PyGSL_Shadow_array(PyObject *shadow, PyObject *master,  enum pygsl_transform_mode mode)
{
	...
	assert(
	    ...
	    ||
	    PyArray_TYPE(m) == PyGSL_TRANSFORM_MODE_SWITCH(mode, NPY_DOUBLE, NPY_FLOAT)
	    );
